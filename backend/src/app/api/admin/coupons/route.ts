...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: true, statusCode: 200, data: { coupons } });
  } catch (error: any) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: true, statusCode: 201, data: { coupon } }, { status: 201 });
  } catch (error: any) {
    return NextResponse.json(
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const { id } = body;
    if (!id) {
      return NextResponse.json({ success: false, statusCode: 400, message: 'Coupon id required' }, { status: 400 });
    }

    const updated = await CouponService.toggleCouponStatus(id);
    return NextResponse.json({ success: true, statusCode: 200, data: { coupon: updated } });
  } catch (error: any) {
    return NextResponse.json(
